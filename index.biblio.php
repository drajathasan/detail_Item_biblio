...
defined('INDEX_AUTH') or die('Ga boleh ngintip :P');
  ...
  <div class="bnt btn-group"><div class="btn btn-info" data-toggle="collapse" data-target="#batchItemDetail" aria-expanded="false" aria-controls="batchItemDetail">' . __('Options') . '</div>';
    ...
    if ($sysconf['index']['type'] == 'index' || $sysconf['index']['type'] == 'sphinx') {
        ...
        if ($can_read AND $can_write) {
            ...
            $datagrid->modifyColumnContent(1, 'callback{showTitleAuthorsSpecial}');
        } else {
        ...
        if ($can_read AND $can_write) {
            ...
            $datagrid->modifyColumnContent(2, 'callback{showTitleAuthorsSpecial}');
        } else {
    ...
    $datagrid->modifyColumnContent(6, 'callback{bisaDiKlik}');
...
<script>
    $(document).ready(function(){
        $('.bisaDiKlik').click(function(){
            let target = $(this).data('target');
            $(`#biblio${target}`).addClass('d-flex');
            $(`#biblio${target}`).slideDown('slow');
        })
    });
</script>
